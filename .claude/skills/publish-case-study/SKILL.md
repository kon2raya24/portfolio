...
3b. **Business-outcomes block** (`<div class="cs-outcomes cs-outcomes--top" data-status="draft">`) directly after the metrics grid. Pairs the engineering-scale numbers above with what *actually changed for the team*. Scaffold it with 3 BEFORE/AFTER pairs pulled from the case-study's own problem + approach sections (do not invent metrics — qualitative wins are fine). Ships with `data-status="draft"` which hides the block via CSS. Flip to `data-status="live"` once the BEFORE/AFTER copy is reviewed and the wording is NDA-safe. Pattern per row: `<div class="cs-outcome-row"><span class="cs-outcome-row__tag cs-outcome-row__tag--before">BEFORE</span><p class="cs-outcome-row__txt cs-outcome-row__txt--before">…</p></div>` then a paired `--after` row.
...
6. `<aside class="cs-cta">` close-out — `<h3>Need similar?</h3>` + 1-sentence positioning paragraph + a `.cs-cta__btns` block with **3 buttons in this exact order**: (1) primary `https://cal.com/lemmuel-turaya/intro` "Book a 15-min call →" with `target="_blank" rel="noopener noreferrer"`, (2) ghost `mailto:turayalemmuel@gmail.com` "Or email me", (3) ghost `../` "View full portfolio". The cal.com primary action is the conversion path — never reorder or drop it.
